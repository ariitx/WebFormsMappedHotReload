...
namespace WebFormsMappedHotReload
{
    ...
    public sealed class AutoSaveFilePackage : AsyncPackage
    {
        ...
        {
            //determine debugger is attached
            if (document?.DTE?.Debugger?.CurrentProcess != null)
            {
                if (document?.Language == "WebForms" && document?.Saved == true)
                {
                    ...
                    //if (window != null && window.Document.FullName == filepath)
                    //{
                    //    var data = window.Document.Saved;
                    //    var current  = System.IO.File.ReadAllBytes(filepath).ToList();
                    //    var temp = new byte[] { };
                    //    current.CopyTo(temp);
                    //    var tmplist = temp.ToList();
                    //    tmplist.Add(0);
                    //    System.IO.File.WriteAllBytes(filepath, tmplist.ToArray());
                    //    System.IO.File.WriteAllBytes(filepath, current.ToArray());
